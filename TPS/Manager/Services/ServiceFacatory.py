...
class ServiceFactory(object):
    def Instance(self,name_service):
        ...
        if name_service == "MLPNN": return MLPNN()
...
class MLPNN(ServiceFactory):

    def request(self,data,options,ip):
        #training or predict
        if 'action' in options: action = options['action'] 
        else: action = "training"

        body = {'data': data, "params" :options['params'] }
        url = 'http://'+ip+'/mlprnn/'+action
        headers = {'Content-type': 'application/json', 'Accept': 'text/plain'}
        LOGER.error(url)
        result = api.post(url, data=json.dumps(body), headers=headers)
        RES = result.json()
        if  RES['status']=="OK":
            return RES
        else:
