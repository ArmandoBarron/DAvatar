...
def storeValuesApp(app_name,app_description,containers_info,app_td_schema,json_structure,extras):
	containers_id = []
...
def storeValuesConts(id_container,id_long,name,status,volumes,platform,image,description,docker_port,host_port,entrypoint,td_schema_pub,td_schema_priv,image_p,volumes_p,status_p,utility_p,extras):
	f = open('./tdschemes_private/'+id_container+'.json', "w")
...
def getExtraTDSchema(td_schema,app,name):
	...
	extras_e = []
	try:
		with open('./config_files/'+str(app)+"-extra.cfg") as f:
			content = f.read()
...
def getStructureFile(file):
	app_structure = []
	if os.path.exists("./structure_files/"+file):
		f = open("./structure_files/"+file, "r")
		content = f.read()
...
def getExtras(file):
	extras = []
	if os.path.exists("./config_files/"+file):
		f = open("./config_files/"+file, "r")
		content = f.read()
		content = content.split("\n")
		for i in range(0,len(content)):
...
storeValuesApp(app_name,app_description,containers_info,app_td_schema,json_structure,extras)
